...
use Manno\DoctrineRestDriver\Annotations\Reader;
 ...
 *
 * @coversDefaultClass Manno\DoctrineRestDriver\Annotations\Reader
 */
class ReaderTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $reader   = new Reader();
        $class    = new \ReflectionClass('Manno\DoctrineRestDriver\Tests\Entity\TestEntity');
        $expected = new Select([
            'value' => 'http://127.0.0.1:3000/app_dev.php/mockapi/products'
        ]);

        $this->assertEquals($expected, $reader->read($class, 'Manno\DoctrineRestDriver\Annotations\Select'));
    }
}

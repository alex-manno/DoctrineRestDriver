...
namespace Manno\DoctrineRestDriver\Tests;

use Manno\DoctrineRestDriver\Tests\Entity\AssociatedEntity;
use Manno\DoctrineRestDriver\Tests\Entity\TestEntity;
...
class FunctionalTest extends WebTestCase
{
     ...
     * @test
     * @group  functional
     * @covers Manno\DoctrineRestDriver\Driver
     * @covers Manno\DoctrineRestDriver\Connection
     * @covers Manno\DoctrineRestDriver\Statement
     * @covers Manno\DoctrineRestDriver\Statement::<private>
     * @covers Manno\DoctrineRestDriver\MetaData
     */
    public function find()
    {
        $entity = $this->em->find('Manno\DoctrineRestDriver\Tests\Entity\TestEntity', 1);
    ...
    {
        $this->em->find('Manno\DoctrineRestDriver\Tests\Entity\TestEntity', 2);
    }
    ...
    {
        $entity = $this->em->getRepository('Manno\DoctrineRestDriver\Tests\Entity\TestEntity')->findOneBy(['id' => 1]);
        $this->assertSame(1, $entity->getId());
        $this->assertSame('MyName', $entity->getName());
        $this->assertSame('MyValue', $entity->getValue());
    }
    ...
    {
        $entity = $this->em->getRepository('Manno\DoctrineRestDriver\Tests\Entity\TestEntity')->findBy(['id' => 1]);
    ...
    {
        $entity = $this->em->getRepository('Manno\DoctrineRestDriver\Tests\Entity\TestEntity')->findAll();
        $this->assertTrue(is_array($entity));
    ...
    {
        $entity = $this->em
            ->createQuery('SELECT p FROM Manno\DoctrineRestDriver\Tests\Entity\TestEntity p WHERE p.id = 1')
            ->getSingleResult();
    ...
    public function nativeQuery()
    {
        ...
        $mapping->addEntityResult('Manno\DoctrineRestDriver\Tests\Entity\TestEntity', 'products');
    ...
    {
        $entity = $this->em
            ->createQuery('SELECT p FROM Manno\DoctrineRestDriver\Tests\Entity\TestEntity p ORDER BY p.name DESC')
            ->getResult();
    ...
    {
        $entity = $this->em
            ->createQuery('SELECT p FROM Manno\DoctrineRestDriver\Tests\Entity\TestEntity p WHERE p.name = ?1')
    ...
    {
        $this->em->find('Manno\DoctrineRestDriver\Entity\NonImplementedEntity', 1);
    }
}
